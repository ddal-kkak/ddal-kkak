      ...
      <form onSubmit={methods.handleSubmit(handleSubmit)}>
        <div className="relative w-full flex gap-5 flex-row-reverse">
            ...
            <ul className="w-full grid grid-cols-2 gap-4">
              {BlockAddButtonList.map((button) => (
                <li key={button.id} className="w-full">
                  <Button type={'button'} variant="ghost" className="w-full aspect-square" onClick={() => append(button.defaultValue as any)}>
                    {button.text}
                  </Button>
                </li>
              ))}
          ...
          <div className="w-full basis-3/4 min-h-[70vh] flex flex-col gap-10">
            ...
            <div className={'space-y-4'}>{fields.map((field, index) => getFieldComponent(field, index))}</div>
          </div>
        </div>
      </form>
    </FormProvider>
